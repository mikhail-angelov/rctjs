...
function createElement(node, isSVG) {
  if (typeof node === "string") {
    ...
    var element = (isSVG = isSVG || "svg" === node.type) ? document.createElementNS("http://www.w3.org/2000/svg", node.type) : document.createElement(node.type);
...
function patchElement(parent, element, oldNode, node, isSVG, nextSibling) {
  if (oldNode == null) {
    ...
    let childNodes = [];(element.childNodes || []).forEach(element => childNodes.push(element));
    let oldNodeIdex = 0;
    if (node.children && node.children.length > 0) {
      for (var i = 0; i < node.children.length; i++) {
        if (oldNode.children && oldNodeIdex <= oldNode.children.length && (node.children[i].type && node.children[i].type === oldNode.children[oldNodeIdex].type || !node.children[i].type && node.children[i] === oldNode.children[oldNodeIdex])) {
          patchElement(element, childNodes[oldNodeIdex], oldNode.children[oldNodeIdex], node.children[i], isSVG);
          oldNodeIdex++;
        } else {
          let newChild = element.insertBefore(createElement(node.children[i], isSVG), childNodes[oldNodeIdex]);
...
function app(selector, view, initProps) {
...
function addChild(acc, node) {
  ...
  } else if (null == node || true === node || false === node) {} else {
